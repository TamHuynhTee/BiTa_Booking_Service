...
export const CreateBranchSchema = yup.object().shape({
    ...
    ward: yup.string().required('Chưa chọn phường, xã của chi nhánh'),
    district: yup.string().required('Chưa chọn quận, huyện của chi nhánh'),
    province: yup.string().required('Chưa chọn tỉnh, thành của chi nhánh'),
    headquarter: yup.boolean().default(false),
});
